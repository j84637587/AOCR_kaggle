...
from dataset_torch import build_dataloader, permute_and_expand_with_zeros
from progressmeter import ProgressMeter, AverageMeter

from losses.BCEDiceLoss import DiceLoss
from models.EfficeientNet3d_v2 import EfficeientNet3d
...
parser.add_argument(
    ...
    metavar="ARCH",
    default="cnn3d",
    help="model architecture (default: efficientnet-b5)",
)
parser.add_argument(
    "-j",
    "--workers",
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = EfficeientNet3d(num_classes=50, in_channels=1, name="efficientnet-b0")
    ...
    # root_directory = "data/preprocess/256x256x50_r/1_Train,Valid_Image"
    root_directory = "data/preprocess/256x256x50_r/1_Train,Valid_Image"
    ...
    logger.info(f"=> csv_ground_truth {csv_ground_truth}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        writer.add_scalar("train_dice_loss", train_dice_loss, epoch)
        writer.add_scalar("train_acc", train_acc, epoch)
        writer.add_scalar("val_loss", val_loss, epoch)
        writer.add_scalar("val_dice_loss", val_dice_loss, epoch)
        ...
        writer.add_scalar("val_auroc", val_auroc, epoch)
        writer.add_scalar("val_TP", total_tp, epoch)
        writer.add_scalar("val_FP", total_fp, epoch)
        writer.add_scalar("val_TN", total_tn, epoch)
        writer.add_scalar("val_FN", total_fn, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, logger, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        dice_losses.update(m_dice(output, target).item())
...
def plot_history(history, log_path, show=False):
    ...
    for i, name in enumerate(["auroc"]):
        ax[2][i].set_title(name)
        ax[2][i].plot(history["val"][name], label=f"val {name}")
        ax[2][i].set_xlabel("Epoch")
        ax[2][i].set_ylabel(name.capitalize())
        ax[2][i].legend()
        ax[2][i].xaxis.set_major_locator(plt.MaxNLocator(integer=True))
