...
class ConvLSTMtransformer_block(nn.Module):
    def __init__(
        ...
        self.transformer_block = TransformerBlock(
            embed_dim=lstm_length, num_heads=4, dropout=0.1, forward_expansion=4
        )
...
class MultiScaleConvLSTMtransformer(nn.Module):
    def __init__(
        self, args, backbone_channels, kernel_size=1, padding=0, batch_first=True
    ):
        super().__init__()
        assert args.roi_x == args.roi_y, "ROI x and y must be the same"
        conv_list = []
        for i, channel in enumerate(backbone_channels):
            ...
            conv_list.append(
                ConvLSTMtransformer_block(
                    lstm_length, channel, channel, kernel_size, padding, batch_first
                )
            )
...
class MyFlexibleUNet2dMultiScaleLSTM(nn.Module):
    def __init__(self, args):
        ...
        self.multi_scale_conv_lstm = MultiScaleConvLSTMtransformer(
            args=args, backbone_channels=backbone_channels
        )
        self.sig = nn.Sigmoid()
...
class MyFlexibleUNet3dMultiScaleLSTM(nn.Module):
    def __init__(self, args):
        ...
        # Add MultiScaleConvLSTM layer after the last convolution layer in the encoder
        backbone_channels = encoder_feature_channel[f"efficientnet-{args.eff}"]
        self.multi_scale_conv_lstm = MultiScaleConvLSTM(
            args=args, backbone_channels=backbone_channels
        )
        self.conv1 = Convolution(
            spatial_dims=3,
            in_channels=1,
            out_channels=1,
            kernel_size=(1, 1, args.num_channel),
            strides=1,
            padding=0,
        )
...
class Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        last, decoder = self.decoder(
            feature=encoder[-1], skip=encoder[:-1][::-1] + [None]
        )
